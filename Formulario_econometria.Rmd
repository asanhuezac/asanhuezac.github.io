...
$$Cov[y_t,y_{t-s}]=\begin{cases}
    \sigma_\varepsilon^2 \sum_{j=0}^{q-s} \theta_j \theta_{j+s} & \text{si } s\leq q \hspace{3mm} donde \hspace{2mm} \theta_0=1\\
    0 & \text{si } s> q \\
  \end{cases}$$

$$Corr[y_t,y_{t-s}]=\begin{cases}
\frac{\sum_{j=0}^{q-s} \theta_j \theta_{j+s}}{(1+\sum_{i=1}^{q} \theta_i^2)} & \text{si } s\leq q \hspace{3mm} donde \hspace{2mm} \theta_0=1\\
    0 & \text{si } s> q \\
  \end{cases}$$
...
$$Dickey-Fuller \hspace{1mm} Aumentado \Rightarrow \Delta y_t=\mu+\beta t+\gamma y_{t-1}+\rho_1\Delta y_{t-1}+\rho_2\Delta y_{t-2}+...+\rho_p\Delta y_{t-p}+u_t$$
